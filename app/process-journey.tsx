...
export default function ProcessJourney({ steps }: { steps: Step[] }) {
  ...
  return (
    <ol ref={ref} className="relative mt-16 space-y-10 sm:space-y-12">
      {/* the spine — hairline track plus a scroll-linked fill in ink */}
      <div
        aria-hidden="true"
        className="absolute left-[23px] top-3 bottom-3 w-px bg-hairline"
      />
      <motion.div
        aria-hidden="true"
        style={{ scaleY: reduce ? 1 : scaleY }}
        className="absolute left-[23px] top-3 bottom-3 w-px origin-top bg-ink/30"
      />
      ...
      {steps.map((step) => (
        ...
        >
          <span className="relative z-10 grid h-12 w-12 place-items-center rounded-full border border-hairline bg-surface font-display text-base font-semibold text-muted">
            {step.n}
          </span>
